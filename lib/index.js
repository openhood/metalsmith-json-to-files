...
/**
 * Builds a string out of a pattern if provided.
 * e.g.
 *   :fields.layout
 *   might return: 'layout-1.html'
 *
 * @param {String} config_pattern
 * @param  {Object} entry
 *
 * @return {String}
 */
var build_from_pattern = function build_from_pattern (config_pattern, entry) {

    log('Building from pattern from: %s', config_pattern);
    ...
    if (config_pattern) {
        ...
        if (get_params(pattern).join('') === '') {
            ...
            throw new TypeError("Couldn't build layout from: " + pattern);
        }
    }
    return result;
};
    ...
    return function (files, metalsmith, done) {
        ...
        var process_file = function process_file (file) {
            ...
            json.forEach(function (element) {
                ...
                // Allow layout to be configured via data
                var layout = build_from_pattern(data.layout_pattern, data);
                if (layout!=null) {
                    data["layout"] = layout;
                }
